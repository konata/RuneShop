...
test("enables updates only for systemd-managed instances", async () => {
  const unmanaged = await new Updater(configuration({ managed: false, updateRef: "HEAD" })).status(false);
  const managed = await new Updater(configuration({ managed: true, updateRef: "HEAD" })).status(false);
